...
export default class IndexSectionList extends PureComponent {
        ...
        for (let i = 0; i < sections.length; i++) {
            array.push(
                <View
                    ...
                    ref={'sectionItem' + i}>
                    <Text
                        style={styles.sectionItem}>{sections[i]}</Text>
                </View>)
        }
        return array;
    }
    ...
    detectAndScrollToSection = (e) => {
        ...
        if (this.lastSelectedIndex !== index && index < sections.length) {
            this.lastSelectedIndex = index;
            this.onSectionSelect(sections[index], index, true);
            this.setState({ text: sections[index], isShow: true });
        }
    }
